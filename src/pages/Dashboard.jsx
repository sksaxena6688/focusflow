...
import { getHabits, getHabitLogs, getTasks, getSessions, getResources } from '../utils/storage';
import { today, lastNDays, formatDuration, calcStreak } from '../utils/dateHelpers';

export default function Dashboard() {
    const [data, setData] = useState({ habits: [], logs: [], tasks: [], sessions: [], resources: [] });

    useEffect(() => {
        setData({
            habits: getHabits(), logs: getHabitLogs(),
            tasks: getTasks(), sessions: getSessions(), resources: getResources(),
        });
    }, []);
    ...
    const todayDone = data.habits.filter(h => data.logs.some(l => l.habitId === h.id && l.date === todayStr)).length;
    const pendingTasks = data.tasks.filter(t => !t.completed).length;
    const todayMinutes = data.sessions.filter(s => s.date === todayStr).reduce((s, x) => s + Number(x.duration || 0), 0);
    const weekMinutes = data.sessions.filter(s => last7.includes(s.date)).reduce((s, x) => s + Number(x.duration || 0), 0);
    ...
    const upcomingTasks = data.tasks.filter(t => !t.completed)
        .sort((a, b) => (a.dueDate || '9999') < (b.dueDate || '9999') ? -1 : 1).slice(0, 5);
    ...
    return (
        <div>
            {/* Header */}
            <div className="mb-7">
                <h2 className="text-2xl font-bold text-[#1A1A2E] tracking-tight">Good {greeting()}, Scholar 👋</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                {/* Today's Habits */}
                <Card className="p-5">
                    <CardHeader title="Today's Habits" action={<Link to="/habits" className="text-xs text-[#2C4A7C] hover:underline">View all →</Link>} />
                        ...
                        data.habits.slice(0, 6).map(h => {
                            ...
                            return (
                                <div key={h.id} className="flex items-center gap-3 mb-2.5">
                                    ...
                                    <span className={`text-sm ${done ? 'line-through text-[#7A7A8A]' : 'text-[#1A1A2E] font-medium'}`}>{h.name}</span>
                                </div>
                            );
                        })
                    )}
                ...
                {/* Upcoming Tasks */}
                <Card className="p-5">
                    <CardHeader title="Upcoming Tasks" action={<Link to="/tasks" className="text-xs text-[#2C4A7C] hover:underline">View all →</Link>} />
            ...
            {/* Today's Study */}
            <Card className="p-5">
                <CardHeader title="Today's Study Sessions" action={<Link to="/study" className="text-xs text-[#2C4A7C] hover:underline">Log session →</Link>} />
                {todaySessions.length === 0 ? (
                    <EmptyState message="No study sessions logged today." />
                ) : (
                    <div className="flex flex-wrap gap-3">
                        {todaySessions.map(s => (
                            <div key={s.id} className="flex items-center gap-3 bg-[#EBF0F8] rounded-lg px-4 py-2">
                                <span className="text-sm font-medium text-[#2C4A7C]">{s.subject}</span>
